...
static void NonlinearAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float twoKp, float twoKi, float dt)
{
    ...
    if(bFilterInit == 0) {
        NonlinearAHRSinit(ax,ay,az,mx,my,mz);
        bFilterInit = 1;
    }
...
volatile uint32_t     last, now;               /* 采样周期, 单位 ms */
//函数名：IMUThread(void)
//描述：姿态软件解算融合函数
...
void IMUThread(void)
{
    ...
    NonlinearAHRSupdate(imu.wx, imu.wy, imu.wz,
    ...
    imu.rol=euler[0] * 180.0f / M_PI_F;
    imu.pit=euler[1] * 180.0f / M_PI_F;
